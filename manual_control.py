...
def game_loop(args):
    ...
    try:
        ...
        while True:
            vehicle = world.world.get_actors().filter('vehicle.*')[0]

            vehicle_transform = vehicle.get_transform()
            waypoint = map.get_waypoint(vehicle.get_location())

            car_x, car_y, car_yaw = transform_to_relevant(vehicle_transform)
            waypoint_x, waypoint_y, waypoint_yaw = transform_to_relevant(waypoint.transform)

            # print(f"Car transform: {transform_to_relevant(vehicle_transform)}")
            # print(f"Waypoint transform: {transform_to_relevant(waypoint.transform)}")

            # dx = waypoint_x - car_x
            # dy = waypoint_y - car_y
            # yaw_to_waypoint = math.atan2(dy, dx) * (180.0 / math.pi)
            # yaw_diff = yaw_to_waypoint - car_yaw

            # if yaw_diff > math.pi:
            #     yaw_diff -= 2*math.pi
            # elif yaw_diff < -math.pi:
            #     yaw_diff += 2*math.pi
            target_wp = random.choice(waypoint.next(10.0))
            # print(f"Car transform: {transform_to_relevant(vehicle_transform)}")
            ...
            if current_time - start_time >= 1.5:
                ...
                vehicle_wp_dist = vehicle_location.distance(wp_location)
